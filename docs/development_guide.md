...
- テスト仕様: [test_specification.md](./test_specification.md)
...
5. 再読み込み後、非デフォルト設定があるタブには要操作バッジ（`!`）が出ることがある。Popup を開けば復元を試みる。
...
| 対象 | 開き方 | 主な確認内容 |
|------|--------|--------------|
| Service Worker | **ビューを検証** の Service Worker リンク | メッセージ中継、`tabCapture`、Offscreen 作成、pending 解決 |
...
- `Receiving end does not exist`  
  → Offscreen が存在しない／閉じられた状態でメッセージ転送した可能性。Background は再作成して 1 回リトライする。
- `active stream`  
...
設定の永続化は、キャプチャと `SET_DEVICE` / `SET_VOLUME` が成功したときだけ行われます。失敗時に storage が変わらないのは仕様です。
...
4. Footer の状態が `Capturing` / `NeedsAction` / `Error` のどれか。
5. 一時停止後も Offscreen の DevTools が残るか（無音時に Offscreen が消えると経路が切れる）。
6. Popup からデバイス変更・音量変更のどちらでも再ルーティングできるか。
...
- [ ] `pnpm check` / `pnpm test` / 手動の境界テスト（再生・一時停止・デバイス切替・再起動後バッジ）を実施した
...
5. ロジック変更後は `pnpm test` を実行

### バグ修正リリース例

1. 修正実装・手動テスト（一時停止後のデバイス維持、再起動後バッジ、失敗時の NeedsAction など）
...
| Popup が Restoring のまま | 以前は stale `pending` 固着があり得た。現行は短時間後に NeedsAction へ落ちる想定 |
| Popup 操作が効かない | SW / Offscreen のコンソールに `Receiving end does not exist` が出ていないか。リトライ後も失敗なら再読み込み |
| 失敗後に storage が変わらない | 仕様。適用成功時のみ保存する |
...
これは音声ルーティング不具合とは無関係で、機能本体の致命傷ではありません。対処は用途で選びます。
...
| 2026-08-12 | 現行実装に合わせて移植（`0.0.2`、`pnpm test`、pending / Offscreen リトライ、保存条件） |
| 2026-08-12 | 音量ブースト（最大 500%）と v1.0.0 に合わせて更新 |
